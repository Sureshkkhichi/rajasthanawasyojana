                ...
                <div class="col-12">
                    <div
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('deals.download-pdf', $deal->id) }}" class="btn btn-soft-primary btn-sm"
                                target="_blank">
                            ...
                            <table class="table table-bordered mb-0">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($deal->agent)
                                            <span
                                                class="badge bg-light text-primary border border-primary fs-12">{{ $deal->waiver_code }}</span>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                            ...
                            @if($deal->allotted_inventory_id)
                                <div class="d-flex gap-2 flex-wrap">
                                    <a class="btn btn-success" href="{{ route('deals.allotment-letter', $deal->id) }}"
                                        target="_blank">
                                    ...
                                    <button class="btn btn-info" wire:click="sendSms" wire:loading.attr="disabled"
                                        wire:target="openEmailModal, sendSms, cancelAllotment">
                        ...
                        <div class="card-body">
                            @if($deal->allotted_inventory_id)
                                {{-- Unit is allotted --}}
                                <div class="alert alert-success d-flex align-items-center p-4 border border-success mb-0"
                                    role="alert">
                                    <i class="ri-checkbox-circle-line text-success fs-24 me-3"></i>
                                    <div class="flex-grow-1">
                                        <h5 class="alert-heading text-success mb-1 fw-semibold">Unit Allotment Successful!
                                        </h5>
                                        <p class="mb-0">
                                            The customer has been allotted
                                            @if($deal->allottedInventory?->inventory_type === 'flat')
                                                <strong>Flat Number {{ $deal->allottedInventory->flat_no }}</strong> (Floor:
                                                {{ $deal->allottedInventory->floor }}, Type:
                                                {{ $deal->allottedInventory->unit_type_label }})
                                            @else
                                                <strong>Plot Number {{ $deal->allottedInventory?->plot_no }}</strong> (Type:
                                                {{ $deal->allottedInventory->unit_type_label }})
                                            @endif
                                        </p>
                                    </div>
                                    <button class="btn btn-outline-danger btn-sm ms-3" wire:click="cancelAllotment"
                            ...
                            @else
                                ...
                                <div class="alert alert-warning d-flex align-items-center p-4 border border-warning mb-0"
                                    role="alert">
                                    <i class="ri-alert-line text-warning fs-24 me-3"></i>
                                    <div class="flex-grow-1">
                                        <h5 class="alert-heading text-warning mb-1 fw-semibold">No Unit Allotted</h5>
                                        <p class="mb-0">
                                            No flat or plot has been allotted to this customer yet. Click the button to
                                            choose an available unit and complete the allotment.
                                        </p>
                                    </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mt-4">
                        <div class="card-body p-4">
                            <livewire:components.activity-timeline :dealId="$deal->id" :wire:key="'deal-timeline-' . $deal->id" />
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <form wire:submit.prevent="sendEmail">
                        <div class="modal-body p-4">
                            <div class="alert alert-soft-info d-flex align-items-center mb-4" role="alert">
                                ...
                                <div>
                                    Please select the <strong>Allotment Letter PDF</strong> and <strong>Demand Letter
                                        PDF</strong> saved from your browser print preview to send to the customer.
                                </div>
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-semibold">Customer Email Address <span
                                        class="text-danger">*</span></label>
                                <input type="email"
                                    class="form-control border-0 bg-light @error('email_recipient') is-invalid @enderror"
                                    wire:model="email_recipient" placeholder="Enter customer email address" readonly>
                            ...
                            <div class="row">
                                    ...
                                    <div class="border rounded p-3 bg-light h-100">
                                        <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-1">
                                            <label class="form-label fw-semibold mb-0">
                                                <i class="ri-file-pdf-2-line text-danger me-1"></i> Allotment Letter PDF
                                                <span class="text-danger">*</span>
                                            </label>
                                            <a href="{{ route('deals.allotment-letter', $deal->id) }}" target="_blank"
                                                class="btn btn-soft-success btn-sm py-1 px-2">
                                                <i class="ri-printer-line me-1"></i> View / Print
                                            </a>
                                        </div>
                                        <input type="file"
                                            class="form-control @error('allotment_pdf_file') is-invalid @enderror"
                                            wire:model="allotment_pdf_file" accept=".pdf">
                                        <small class="text-muted d-block mt-1 fs-11">
                                            <i class="ri-arrow-right-s-line me-1"></i>Click <strong>View / Print</strong> &
                                            save as PDF, then select file here.
                                        </small>
                                        @error('allotment_pdf_file') <div class="invalid-feedback d-block">{{ $message }}
                                        </div> @enderror
                                        @if($allotment_pdf_file)
                                            <div class="text-success fs-12 mt-2 fw-semibold"><i
                                                    class="ri-checkbox-circle-fill me-1"></i>
                                                {{ $allotment_pdf_file->getClientOriginalName() }}</div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    <div class="border rounded p-3 bg-light h-100">
                                        <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-1">
                                            <label class="form-label fw-semibold mb-0">
                                                <i class="ri-file-pdf-2-line text-danger me-1"></i> Demand Letter PDF <span
                                                    class="text-danger">*</span>
                                            </label>
                                            <a href="{{ route('deals.demand-letter', $deal->id) }}" target="_blank"
                                                class="btn btn-soft-warning btn-sm py-1 px-2">
                                                <i class="ri-printer-line me-1"></i> View / Print
                                            </a>
                                        </div>
                                        <input type="file"
                                            class="form-control @error('demand_pdf_file') is-invalid @enderror"
                                            wire:model="demand_pdf_file" accept=".pdf">
                                        <small class="text-muted d-block mt-1 fs-11">
                                            <i class="ri-arrow-right-s-line me-1"></i>Click <strong>View / Print</strong> &
                                            save as PDF, then select file here.
                                        </small>
                                        @error('demand_pdf_file') <div class="invalid-feedback d-block">{{ $message }}</div>
                                        @enderror
                                        @if($demand_pdf_file)
                                            <div class="text-success fs-12 mt-2 fw-semibold"><i
                                                    class="ri-checkbox-circle-fill me-1"></i>
                                                {{ $demand_pdf_file->getClientOriginalName() }}</div>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="modal-footer bg-light px-4 py-3">
                            ...
                            <button type="submit" class="btn btn-primary px-4" wire:loading.attr="disabled"
                                wire:target="sendEmail, allotment_pdf_file, demand_pdf_file">
